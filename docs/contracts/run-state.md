...
- `run_workflow_from_node` reuses an existing retained session and runs from a selected main-path graph node. It requires the Workflow Settings Run Policy Run from selected toggle, browser retention `retain`, a persistent selected browser profile, and a retained session matching the workflow/profile. Run Policy scope decides whether the compiled sub-plan contains only the selected node or the selected node through the downstream main path. Selected-node compilation resolves Call Subflow nodes the same way as full workflow runs.
