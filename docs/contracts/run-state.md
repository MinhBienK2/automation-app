...
- Outputs can contain evidence generated by the backend, including `__action_traces` for per-action execution mode evidence, `__evidence` for run-scoped screenshot/download metadata, and `fingerprint_preflight` for sanitized probe verdict evidence.
