...
- `browser_identity` includes CloakBrowser wrapper/binary evidence, fingerprint seed hash, non-secret proxy metadata, timezone/locale source, GeoIP/WebRTC policy, active advanced overrides, and behavior fidelity. It must not include proxy passwords, cookies, localStorage, sessionStorage, or raw profile storage.
