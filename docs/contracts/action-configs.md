...
Set Viewport is a runtime viewport-size action. Active authoring exposes only `width` and `height`. Device scale factor, mobile mode, touch capability, and other low-level fingerprint values are generated by CloakBrowser from the stable Browser Launch seed rather than exposed as in-run action fields.
