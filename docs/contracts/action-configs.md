...
- Drag/drop uses `source_target` and `target_target`; custom select uses `trigger_target`. Drag/drop may also set optional `target_position`: missing or `{ mode: "center" }` preserves the legacy drop-to-target-center behavior; `{ mode: "percent", x_percent, y_percent }` drops at a 0-100 percentage point inside the target box; `{ mode: "offset", x_px, y_px }` drops at a pixel offset from the target box's top-left corner.
- Backend validation requires a valid structured target for required element actions, except targetable single-target actions may use `target_ref` from an earlier `find_element` action in the same run. This includes pointer, form, keyboard-targeted, wait element-state, capture, assertion, and Scroll To Element configs. `find_element`, Drag and Drop source/destination targets, custom select trigger targets, and Scroll Until Element Visible keep locator-specific target fields.
...
- Drag/drop authoring presents the serialized `source_target` as the `Drag source` group and presents `target_target` plus `target_position` together under `Drop setup`, keeping source and destination labels distinct while preserving the same persisted config fields.
...
- Runner interaction dispatch uses an internal capability map: CloakBrowser-native paths are preferred for supported element/page/frame APIs, Scroll To Element may use CloakBrowser's exported human scroll helper before falling back to app-owned wheel timing, Scroll Until Element Visible uses app-owned page-scroll search gestures before the same target-scroll path, custom human behavior owns Page Scroll, untargeted key chords, paste shortcut orchestration, and right-click button preservation, and direct DOM is used for read/assert/storage actions or final fallbacks only.
