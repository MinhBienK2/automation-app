...
- Backend validation requires a valid structured target for required element actions, except `click` may use `target_ref` from an earlier `find_element` action in the same run.
...
- Click authoring exposes a target-source segmented control. Use Locator shows structured locator fields; Use Find Element ref shows only `target_ref` plus click readiness fields so hidden locator constraints are not presented as active.
