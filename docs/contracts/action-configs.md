...
Set Viewport is a runtime viewport-size action. Active authoring exposes only `width` and `height`. Device scale factor, mobile mode, and touch capability belong in Workflow Settings Browser Launch's Custom fingerprint overrides before Chromium starts.
