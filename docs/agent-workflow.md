...
4. Read only the docs listed by those routes.
5. Inspect the current source files listed by the route.
6. Use `.agents/skills/test-driven-development` before behavior-changing code.
7. Implement the smallest scoped change.
8. Run focused checks first, then broader checks when the touched area requires it.
9. Update `docs/` when behavior, contracts, routes, ownership, or verification changed.
10. Before final response, confirm docs and code agree for the touched area.
