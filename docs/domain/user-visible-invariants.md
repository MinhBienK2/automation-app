...
- Workflow Settings Browser Launch exposes proxy URL/credentials/bypass and non-secret proxy metadata, timezone, locale, GeoIP, WebRTC policy, a Humanize browser input toggle, a Humanize preset select with `default` and `careful`, optional owned fingerprint preflight, and headless launch controls. CloakBrowser seed-derived platform, hardware, memory, storage, screen, GPU, device, and font defaults are not user-editable Settings fields. Fingerprint preflight requires an allowlisted probe origin and headed mode.
- Set Viewport is an in-run viewport-size action. Active authoring exposes width and height only; low-level device and fingerprint values are generated by CloakBrowser from the stable Browser Launch seed.
...
- Full runs use persisted Workflow Settings as the run baseline. Browser Launch identity settings, including profile directory, fingerprint seed, proxy, timezone, locale, supported WebRTC IP policy values, humanize toggle/preset, preflight, and headless mode, are resolved before browser launch. CloakBrowser seed-derived low-level fingerprint defaults are not manually overridden by Settings. Environment initial variables are applied before the first graph step; saved edge waits compile into synthetic wait steps before target nodes; Run Policy max duration cancels and fails overlong runs with a timeout reason.
