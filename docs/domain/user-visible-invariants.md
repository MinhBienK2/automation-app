...
- Scroll authoring exposes Page Scroll, Scroll To Element, and Wait Then Scroll To Element labels while preserving the serialized `page`, `into_view`, and `until_visible` modes. Page Scroll shows Direction and Pixels; element-targeted modes show Target locator, optional Iframe XPath, and Timeout ms without low-level target constraint or scroll-tuning fields.
