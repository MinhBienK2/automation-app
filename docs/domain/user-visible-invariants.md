...
- Browser recording never exposes captured password or secret-like text field values to the renderer. Those generated input steps are excluded by default with a review warning until the operator supplies a safe literal or variable. Stopping a recorder drains buffered fallback events before review draft generation. Record Replacement cannot start while the target workflow, target browser profile, or batch runner is active. Saving a recording draft only honors reviewed labels, inclusion flags, and supported captured value edits against the backend-held draft steps; renderer-supplied action type or locator replacements are ignored. Discarding a recording session and successfully saving a recording draft consume the backend in-memory recorder state instead of leaving reusable draft/session handles.
