...
- Workflow deletion uses an in-app confirmation dialog that asks whether to keep or delete the workflow's private browser profile data. Deleting profile data is selected by default, and operators can clear the choice to keep retained login state. Deleting profile data removes only unshared inactive profile directories.
...
- Workflow Settings Browser Launch exposes proxy URL/credentials/bypass and non-secret proxy metadata, timezone, locale, GeoIP, optional Custom fingerprint overrides, a Humanize browser input toggle, a Humanize preset select with `default` and `careful`, optional owned fingerprint preflight, and headless launch controls. New browser identities default timezone and locale from the app runtime, and operators can clear or edit those fields. Fingerprint preflight requires an allowlisted probe origin and headed mode.
- Custom fingerprint overrides are off by default so CloakBrowser derives coherent device signals from the fingerprint seed. When enabled, the group exposes user agent, viewport width/height, device scale factor, mobile viewport, touch input, fingerprint platform, hardware concurrency, device memory, storage quota, and fingerprint fonts directory; those values are ignored at launch and by override-specific validation while the group is disabled.
- Set Viewport is an in-run viewport-size action. Active authoring exposes width and height only; device scale factor, mobile viewport, and touch input are launch-time Custom fingerprint overrides when an operator intentionally needs a tested device bundle.
