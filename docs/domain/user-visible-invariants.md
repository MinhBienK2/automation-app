...
- Workflow Settings contains General, Execution, Browser, Behavior, Environment, Variables, Triggers, and Advanced sections. It is per-workflow and distinct from the app-level Settings screen. Settings are saved through a single dialog-level Save Settings action rather than separate section save buttons.
...
- Workflow Settings Behavior exposes Behavior Lab enablement, persona, strictness, profile name, account label, target domains, seed, max actions per minute, and evidence toggles. Enabling Behavior Lab requires an explicit seed, target domains, and Browser challenge policy set to Detect only or Pause for human; realistic, assistive, and stress-test modes also require an account label.
...
- When Behavior Lab is enabled, full runs attach `behavior_report` to run outputs. The report includes profile, account label, target domains, seed, action telemetry, summary score, severity, and anomaly codes such as high action rate, missing click-offset telemetry, uniform timing, or incomplete telemetry.
