...
- Scroll authoring exposes Page Scroll and Scroll To Element labels while preserving the serialized `page` and `into_view` modes. Page Scroll shows Direction and Pixels; element-targeted scroll shows Target locator, optional Iframe XPath, and Timeout ms defaulting to `60000` without low-level target constraint or scroll-tuning fields.
