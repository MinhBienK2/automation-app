...
- Workflow Settings Browser Launch exposes proxy URL/credentials/bypass and non-secret proxy metadata, timezone, locale, GeoIP, viewport/device, browser brand selection, allowlisted advanced fingerprint overrides, a Humanize browser input toggle, a Humanize preset select with `default` and `careful`, optional owned fingerprint preflight, and headless launch controls. Browser brand selection runs through CloakBrowser's Chromium core: Chrome uses the default CloakBrowser Chromium-compatible identity, Microsoft Edge maps to CloakBrowser's supported Edge brand flag, and Firefox remains an unsupported planning option with a validation warning. Fingerprint preflight requires an allowlisted probe origin and headed mode.
- Workflow Settings validation warns operators when proxy-enabled identities lack explicit timezone/locale and GeoIP is off, when a custom user agent may diverge from UA Client Hints, when a shared fingerprint fonts directory can create a stable font hash, and when macOS fingerprint profiles need owned preflight because of upstream CloakBrowser caveats.
...
- `browser_identity` output evidence includes a fingerprint seed hash, selected browser brand, non-secret network metadata, timezone/locale source, supported WebRTC policy, active advanced overrides, configured humanization status and preset, and CloakBrowser wrapper/binary version evidence.
