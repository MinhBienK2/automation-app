...
- Workflow Settings contains General, Run Policy, Browser Launch, Graph Defaults, and Environment sections. It is per-workflow and distinct from the app-level Settings screen. Settings are saved through a single dialog-level Save Settings action rather than separate section save buttons.
...
- Workflow Settings Graph Defaults exposes the default link wait for newly created graph links. It supports no default wait, fixed duration milliseconds, or random min/max milliseconds, and changing it must not rewrite existing links.
...
- New graph edges copy the saved Graph Defaults link wait at creation time. Edge waits are duration-only transition delays and compile before the target node; explicit Wait and Random Wait nodes remain the user-visible choice for page-state waits and named workflow pauses.
