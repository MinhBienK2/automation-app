...
- Scroll authoring exposes Page Scroll, Scroll To Element, and Scroll Until Element Visible labels while preserving the serialized `page`, `into_view`, and `until_element_visible` modes. Page Scroll shows Direction and Pixels; Scroll To Element shows Target locator, optional Iframe XPath, and Timeout ms defaulting to `60000`; Scroll Until Element Visible also shows Direction and Pixels for the repeated page-scroll search gesture without low-level target constraint or scroll-tuning fields.
