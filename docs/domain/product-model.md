...
- Workflow Settings is the per-workflow configuration aggregate for workflow identity, execution defaults, browser launch profile, Behavior Lab policy, environment defaults, initial variables, planned trigger metadata, and advanced compatibility diagnostics.
...
- The Behavior section of Workflow Settings owns Behavioral Analytics Lab enablement, persona, strictness, named test account label, target domain allowlist, deterministic seed, timing/velocity budget, and evidence report policy. When enabled, runs emit a `behavior_report` output with telemetry, score, severity, and anomaly evidence without changing the workflow graph's intended actions.
...
- Configure Behavioral Analytics Lab policy and evidence reporting for authorized owned-domain runs.
