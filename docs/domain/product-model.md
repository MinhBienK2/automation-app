...
- The Browser Launch section is identity-oriented even while the tab label remains compatible. New workflows automatically get a browser identity with a stable `identity_id`, editable display name, stable `profile_dir`, and fixed CloakBrowser fingerprint seed. Reuse login session only controls persistent storage; it does not rotate the fingerprint identity. The section also owns Run from selected enablement, proxy posture, timezone/locale/GeoIP, viewport/device flags, humanization, optional owned fingerprint preflight, and headed/headless policy. Legacy browser config commands map to this section for compatibility without changing identity metadata.
