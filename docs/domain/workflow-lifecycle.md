...
- Set Variables edits multiple typed rows in one node while keeping legacy single `{ name, value }` saved configs loadable. Set JSON Variables accepts a JSON object and flattens nested object keys into dot-path runtime variables while preserving arrays whole.
