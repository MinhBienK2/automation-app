...
- `run_workflow` loads and validates saved Workflow Settings, applies Browser Launch identity settings before launch including profile directory, fixed fingerprint seed, proxy, timezone, locale, supported WebRTC policy values, humanize toggle/preset, and headless mode, while leaving low-level platform, hardware, memory, storage, screen, GPU, device, and font values to CloakBrowser seed defaults. It optionally runs owned fingerprint preflight before graph actions, prepends Environment initial variables before the first graph step, compiles edge delays as synthetic wait steps before their target nodes, promotes graph domain allowlists into a pre-navigation run policy, enforces maximum workflow duration, and applies browser retention as the default terminal session policy. Authors use explicit Wait and Random Wait nodes when a workflow needs a business-semantic pause.
