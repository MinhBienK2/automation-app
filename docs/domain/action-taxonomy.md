...
- Browser Context: dialogs, downloads, runtime viewport size, geolocation, permissions, and other runtime-safe browser context actions. Low-level device and fingerprint values are generated by CloakBrowser from the stable Browser Launch seed, not edited through in-run Set Viewport behavior.
