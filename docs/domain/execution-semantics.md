...
- Scroll Page mode uses custom human wheel chunks with bounded pauses and occasional small reverse corrections because CloakBrowser does not patch `page.mouse.wheel` directly; `window.scrollBy` is only a fallback for driver adapters without wheel input. Scroll To Element and Wait Then Scroll To Element resolve the target, then use a goal-directed wheel planner with bounded random chunk sizes, pauses, and occasional small reverse corrections until the target is sufficiently visible. `scrollIntoViewIfNeeded` is not the primary path for Scroll actions.
