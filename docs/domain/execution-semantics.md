...
- Workflow Settings Browser Launch resolves the browser identity before the browser starts. It maps persistent versus temporary storage, stable profile directory, fingerprint seed, browser brand, proxy server/bypass/credentials, timezone, locale, GeoIP, viewport/device flags, supported WebRTC policy values, allowlisted advanced fingerprint overrides, humanize toggle/preset, and headless mode into CloakBrowser launch options. Chrome uses CloakBrowser's default Chromium-compatible brand, Microsoft Edge maps to CloakBrowser's supported Edge brand flag, and Firefox is stored as an unsupported planning option that currently runs Chrome-compatible CloakBrowser with a validation warning. For headed runs, the runner also passes a `--window-size=<viewport width>,<viewport height>` Chromium argument so the outer window starts aligned with the configured viewport identity.
