...
- Browser identity, proxy, Custom fingerprint override values, profile, and download behavior belong in Workflow Settings Browser Launch, not in in-run action nodes.
- `set_viewport` changes only runtime viewport width and height. Device scale factor, mobile mode, and touch capability are launch-time Custom fingerprint overrides and are ignored while that optional group is disabled.
