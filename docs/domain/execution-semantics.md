...
- Workflow Settings Browser Launch resolves the browser identity before the browser starts. `BrowserSessionManager` maps persistent versus temporary storage, stable profile directory, fingerprint seed, fingerprint fonts directory, proxy server/bypass/credentials, explicit timezone/locale or local machine timezone/locale, GeoIP, supported WebRTC policy values, humanize toggle/preset, and headless mode into CloakBrowser launch options. Blank timezone/locale fields use the detected local machine values when GeoIP is off; when GeoIP is on and the fields are blank, GeoIP owns those values. Persona viewport/window dimensions stay in sanitized identity evidence for audit and preflight comparison, but Browser Launch no longer sends explicit `viewport`, `userAgent`, or persona-derived `--window-size` overrides; CloakBrowser's fingerprint seed owns those launch-level device signals. In-run Set Viewport can still change runtime viewport later.
