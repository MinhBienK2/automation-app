...
- Scroll Page mode uses custom wheel gestures because CloakBrowser does not expose a page-distance scroll helper; `window.scrollBy` is only a fallback for driver adapters without wheel input. Page Scroll defaults to the `human_like` style, which calculates a small number of decisive chunks from the requested pixel distance while preserving the exact total. Each chunk is emitted as a short eased burst of smaller wheel pulses with random sub-pulse pauses and a longer random pause before the next chunk. Page Scroll can also use `smooth_single`, which sends one wheel event for the requested distance and only falls back to smooth DOM scrolling when wheel input is unavailable. Scroll To Element resolves the target and first tries CloakBrowser's exported human scroll helper when the Playwright page exposes the required mouse and viewport primitives; if the helper is unavailable or fails, the runner falls back to the app-owned wheel planner. Scroll Until Element Visible is for lazy-load/infinite-scroll pages: it checks whether the target has a visible bounding box, page-scrolls in the configured direction and pixel amount when missing, repeats until timeout or max attempts, then uses the same target-scroll path once the target exists. The planner recalculates chunk size and pause duration from the remaining distance until the target is sufficiently visible, scrolling down or up depending on whether the target is below or above the viewport. Scroll movement stays monotonic toward the goal to avoid jittery up/down oscillation. `scrollIntoViewIfNeeded` is not the primary path for Scroll actions.
...
- `extract_regex_matches` reads an existing run output, applies a backend-validated regex, stores all matches in an output list, and can append/dedupe across loop iterations.
- `write_text_file` writes a run output to a safe text artifact under the current run evidence directory and stores the evidence-relative file path in outputs.
...
- Step progress reports current step id/number and completed step ids. Graph branch/body actions keep their source node ids in the compiled run plan, so nested `If`, loop, retry, and related branch nodes can appear as active/completed on the canvas before continuation nodes run. Repeated nested graph actions append repeated completed ids for each loop/retry occurrence so run monitors can show a log entry for every activity, while graph canvas state can still treat completed ids as a membership set.
...
- Retained browser sessions are keyed by workflow/profile so multiple isolated workflows can retain inspectable browsers at the same time. Starting a fresh persistent-profile run closes any retained session that already owns the same profile directory before a new CloakBrowser context launches, releasing that persistent profile lock while preserving retained sessions for other profile directories.
