...
- Scroll Page mode uses custom human wheel gestures because CloakBrowser does not patch `page.mouse.wheel` directly; `window.scrollBy` is only a fallback for driver adapters without wheel input. Page Scroll calculates a small number of decisive chunks from the requested pixel distance while preserving the exact total. Each chunk is emitted as a short eased burst of smaller wheel pulses with random sub-pulse pauses and a longer random pause before the next chunk. Scroll To Element and Wait Then Scroll To Element resolve the target, then use a goal-directed wheel planner that recalculates chunk size and pause duration from the remaining distance until the target is sufficiently visible. The same planner scrolls down or up depending on whether the target is below or above the viewport. Scroll movement stays monotonic toward the goal to avoid jittery up/down oscillation. `scrollIntoViewIfNeeded` is not the primary path for Scroll actions.
