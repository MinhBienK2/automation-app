...
|---|---|---|---|
| `buildLaunchOptions` | top-level CloakBrowser option mapping, args, context options | `electron/backend/runner.test.ts` | reviewed for launch mapping; F-001 closed for WebRTC policy mapping/rejection; stronger user-agent UI coverage remains |
...
| Browser context runtime actions | cookie set/clear, headers, permissions, geolocation, viewport size, and launch-time device-shape rejection | `runner.test.ts`, `graphCompiler.test.ts`, `ActionConfigEditor.test.tsx`, `stepHelpContent.test.ts`, `browser-context-storage.e2e.ts` | reviewed for cookie/header/permission/geolocation and viewport width/height; F-007 closed by explicit size-only UI/help plus compiler/runner rejection coverage; F-018 closed by blank-domain current-host runner coverage |
| Advanced network/storage actions | JS output, strict-humanized guard, request/response wait, route block/mock, web storage | `runner.test.ts`, `graphCompiler.test.ts`, `capture-network.e2e.ts`, `browser-context-storage.e2e.ts` | reviewed for listed behavior; F-008 closed by runner substring matching test; F-019 closed by runner execute-js timeout test |
...
| Element/form/pointer/keyboard/assertion actions | text input, click, pointer, form, checkbox, keyboard, clipboard, assertions | `core-execution.e2e.ts`, `extended-form-actions.e2e.ts`, `pointer-actions.e2e.ts`, `keyboard-dialog.e2e.ts`, `wait-assertion-actions.e2e.ts`, `runner.test.ts`, `graphCompiler.test.ts` | reviewed for visible behavior; F-010 closed by runner pass/fail coverage for every `assert_element` state; F-021 closed by runner advanced-click coverage; F-023 closed by graph compiler and runner invalid-enum tests; F-011: legacy `set_checkbox` compatibility coverage missing; F-020/F-022: scroll/clear advanced fields have UI/runtime/help drift |
...
- F-001: `electron/backend/commands.test.ts` rejects unsupported `disabled_if_supported`; `electron/backend/runner.test.ts` asserts supported WebRTC policy launch args and evidence for `default`, `auto_proxy_exit_ip`, and `explicit_ip`.
- F-007: `electron/backend/runner.test.ts` and `electron/backend/graphCompiler.test.ts` reject non-default Set Viewport device-shape fields; `ActionConfigEditor.test.tsx` and `stepHelpContent.test.ts` prove active authoring/help only expose width and height.
- F-008: `electron/backend/runner.test.ts` asserts `mock_response.url_contains` matches a full request URL by substring before fulfilling the response.
- F-010: `electron/backend/runner.test.ts` asserts pass/fail behavior for every supported `assert_element` state.
- F-018: `electron/backend/runner.test.ts` asserts blank `set_cookie.domain` infers the current page host after navigation.
- F-019: `electron/backend/runner.test.ts` asserts `execute_js.timeout_ms` fails overlong page evaluation before storing output.
- F-021: `electron/backend/runner.test.ts` asserts Click `force_dom`, scroll alignment, and offset position behavior.
