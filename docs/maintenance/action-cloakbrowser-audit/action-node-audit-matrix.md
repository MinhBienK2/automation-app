...
| `assert_element` | implemented_partial_requires_validation | `src/types/workflow.ts:49` | yes | yes | yes | token-check | yes | yes | yes | matrix | yes | reviewed; F-010 |
...
| `set_viewport` | implemented | `src/types/workflow.ts:75` | yes | yes | yes | token-check | yes | yes | no | matrix | yes | reviewed; F-007 |
...
| `execute_js` | implemented | `src/types/workflow.ts:85` | yes | yes | yes | token-check | yes | yes | yes | matrix | yes | reviewed |
...
| `mock_response` | implemented | `src/types/workflow.ts:89` | yes | yes | yes | token-check | yes | yes | no | matrix | yes | reviewed; F-008 |
...
| `input_text` | `config.typing_mode` / `config.delay_ms` | enum plus optional number | optional typing fidelity fields | omitted from visible default | not visible in primary editor; update helper supports both fields | neither typing-mode enum nor delay range is validated for `input_text` | preserved | `runner.ts:586` through `runner.ts:591` uses type mode and delay; otherwise fill | detailed help describes Typing mode; visible field list omits it | runner happy path covers `typing_mode: type` | reviewed; F-023 |
| element action timing | `config.wait_until` / `config.timeout_ms` / `config.retry_interval_ms` | readiness enum and optional positive/nonnegative numbers | optional API/runtime readiness fields | omitted from visible defaults | `ElementOptionalFields` exists but is not rendered by `ActionConfigEditor`; per-action update helpers preserve imported values | `graphCompiler.ts:1095` through `graphCompiler.ts:1101` validates timing numbers but not wait enum | preserved | `locatorForAction` applies wait/readiness for target actions in `runner.ts:1338` through `runner.ts:1356`; unknown wait values fall through | detailed help describes timing fields; visible field lists mostly omit them | `runner.test.ts:2033` covers click/input readiness; UI exposure untested | reviewed; F-023 |
...
| `click` | `config.mode` | `"real" \| "force_dom" \| null` | optional click mode | omitted from visible default | no visible Mode editor found | not enum-validated | preserved | `runner.ts:598` through `runner.ts:602` always performs locator click; force DOM mode is ignored | detailed help documents Real/Force DOM | workflow form update test covers preservation only | reviewed; F-021 |
| `click` | `config.button` / `config.click_count` | pointer options | optional click parameters | omitted from visible default | no visible controls found | timing helper validates neither button enum nor positive click count | preserved | runner passes button and click count to locator click | detailed help documents Button and Click count | right-click action has dedicated runtime test; click button/count boundary coverage missing | reviewed; F-021 |
| `click` | `config.scroll_into_view`, `block`, `inline`, `position`, `offset_x`, `offset_y` | scroll/coordinate options | optional click targeting fields | omitted from visible default | no visible controls found | not validated for enum/range/required-offset combinations | preserved | runner does not reference these fields before locator click | detailed help documents scroll alignment and offset click | workflow form update test covers preservation only | reviewed; F-021 |
...
| `select_option` | `config.match_by` / `config.value` | enum plus required string | required option selector | default label and empty value | select/input in `ActionConfigFormFields.tsx:27` through `ActionConfigFormFields.tsx:47` | value required; match enum not runtime-validated | preserved | `runner.ts:650` through `runner.ts:659` treats anything other than `label` as `value` | help lists Match by and Value | core E2E covers label path | reviewed; F-023 |
| `set_checkbox` | `config.state` | `"checked" \| "unchecked"` | required checkbox state | default checked | state select in form fields | state enum not validated beyond TypeScript | preserved | runner checks only when state is `checked`; every other value unchecks | help lists State | F-011: compatibility coverage for legacy `set_checkbox` remains overstated | reviewed; F-011; F-023 |
...
| `assert_element` | `config.state` | `"attached" \| "visible" \| "hidden" \| "enabled" \| "disabled"` | required assertion state | default visible | state select in output fields | state enum not validated; target/timing validation only | preserved | runner only enforces visible state; other states silently pass | help lists all states | wait/assertion E2E covers visible only | reviewed; F-010 |
| `assert_text` | `config.text` / `match_mode` / `timeout_ms` | required text, match enum, optional timeout | default empty text, contains | output fields render text/match/timeout | text required and timeout positive; match enum not validated | preserved | runner compares only `contains` and `equals`; unknown match modes perform no assertion | help lists fields | E2E covers pass and failure for contains; equals/boundaries weaker | reviewed; F-023 |
...
| `set_cookie` | `config.domain` | `string \| null` | optional domain with UI current-host intent | default null | Domain input placeholder says current host | no validation requires domain or URL fallback | preserved | `runner.ts:1028` passes `undefined` when blank and does not infer current host | help lists Domain only | E2E always supplies hostname, so blank-domain default path is uncovered | reviewed; F-018 |
...
| `set_viewport` | `config.width` / `config.height` | `number` | required viewport dimensions | default `1280x720` | numeric inputs in `ActionConfigSessionFields.tsx:171` through `ActionConfigSessionFields.tsx:191` | `graphCompiler.ts:962` through `graphCompiler.ts:965` validates positive values | preserved | `runner.ts:997` through `runner.ts:1001` calls `page.setViewportSize` | help lists Width and Height | runner and E2E assert width/height | reviewed |
| `set_viewport` | `config.device_scale_factor` | `number \| null` | optional device shape | default `1` | numeric input in `ActionConfigSessionFields.tsx:193` | `graphCompiler.ts:966` validates positive value when present | preserved | not applied by current runner viewport call | help/test labels exist | only update/validation tests, no runtime assertion | reviewed; F-007 |
| `set_viewport` | `config.mobile` / `config.touch` | `boolean` | required device shape flags | default false | select controls in `ActionConfigSessionFields.tsx:211` through `ActionConfigSessionFields.tsx:233` | no runtime type validation beyond TypeScript | preserved | not applied by current runner viewport call | help/test labels exist | runner/E2E assert only viewport size | reviewed; F-007 |
...
| `execute_js` | `config.timeout_ms` | `number \| null` | optional execution timeout | omitted from default but UI shows `1000` fallback | timeout input in `ActionConfigReliabilityFields.tsx:199` | `graphCompiler.ts:1014` validates positive value when present | preserved | `runner.ts:1063` through `runner.ts:1070` never references timeout | help lists Timeout ms through shared advanced fields | no runner/E2E timeout behavior test found | reviewed; F-019 |
...
| `mock_response` | `config.url_contains` | `string` | required URL substring by contract/help | default `/api/mock` | URL contains input in reliability fields | `graphCompiler.ts:1031` requires nonblank value | preserved | `runner.ts:1096` registers it as a raw route pattern, not a substring predicate | help says mock matching response by URL contains | E2E uses full URL, not substring default | reviewed; F-008 |
...
| `assert_output` | `name`, `match_mode`, `value` | output name, match enum, expected value | compatibility output assertion | default output/value/contains | hidden compatibility graph inspector path exists | name/value required; match enum not validated | preserved/compiled from graph node | `runner.ts:974` through `runner.ts:982` checks only equals/contains; unknown modes pass | graph help documents Match | F-005 and F-023 cover missing tests/enum validation | reviewed; F-005; F-023 |
...
- F-007: `set_viewport` exposes `device_scale_factor`, `mobile`, and `touch`, but runtime applies only width and height and existing tests assert only width/height.
...
- F-008: `mock_response.config.url_contains` is documented and validated as contains semantics, but runner registers it as a raw route string and current E2E only uses a full URL.
...
- F-010: `assert_element` exposes five states, but runner only enforces `visible`.
