...
| M2 Runner Vertical Slice | PARTIAL | App API can create a workflow, compile a simple graph, execute runner core events, enforce navigation allowlist in runner tests, register screenshot artifacts, route `startRun`/`cancelRun` through the supervised JSONL runner process with streamed events, and forward persisted run events to the renderer over preload subscription. Real CloakBrowser-backed navigate/fill/click/wait smoke and user-facing run monitor UI remain. |
