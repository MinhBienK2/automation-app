...
| --- | --- | --- | --- |
| 01 Product Model | SPEC DONE | PARTIAL | Canonical concepts are reflected in shared Electron types and docs. Workspace policy, Identity Profile, Run Profile, run, artifact, event, and evidence foundations exist. Full environment model, profile inventory, and UI coverage are not complete. |
...
| 03 Data And Storage | SPEC DONE | IMPLEMENTATION DONE for M1 | `electron/main/storage.ts` initializes the new SQLite workspace schema, workflows, active graph versions, workspace allowed-origin policy, run profiles, run records with terminal status, run events, artifacts, evidence records, identity profiles, soft delete, duplicate, and temporary settings snapshots. Verified by `electron/main/storage.test.ts`. Full environments, secret references, import/export, retention cleanup, run history UI queries, and profile inventories remain. |
...
| 09 UI/UX Feature Parity | SPEC DONE | PARTIAL | Existing React UI can call Electron preload APIs for workflow, graph, settings, run foundation commands, run history, run profiles, identity profiles, workspace policy, evidence export foundations, and run event subscriptions. Full Electron-native parity for run-profile editor, identity editor, evidence viewer, import/export, issue panel, policy editor, and user-facing streamed run monitor remains. |
