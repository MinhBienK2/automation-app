...
| M2 Runner Vertical Slice | PARTIAL | App API can create a workflow, compile a simple graph, execute runner core events, enforce navigation allowlist in runner tests, register screenshot artifacts, route `startRun`/`cancelRun` through the supervised JSONL runner process with streamed events, forward persisted run events to the renderer over preload subscription, and show a compact recent event monitor in the workflow detail status bar. Real CloakBrowser-backed navigate/fill/click/wait smoke remains. |
| M3 Core Feature Parity | PENDING | Existing React UI is bridged to Electron for foundation APIs, including workflow default profile/environment selection, but full P0 parity is not complete. |
| M4 Production Identity And Evidence | PARTIAL | Identity Profile storage, app API, preload contract, coherence validation foundation, workflow default identity snapshot resolution, workspace allowed-origin policy normalization/snapshots, Settings policy editor, workspace `maxConcurrency` enforcement, persistent profile locking, runner fingerprint preflight gate, and sanitized preflight evidence persistence exist. Full UI wiring for saved profiles, real owned-probe smoke, run-profile selection, evidence viewer, and broader operator policy management remain. |
...
| --- | --- | --- | --- |
| 01 Product Model | SPEC DONE | PARTIAL | Canonical concepts are reflected in shared Electron types and docs. Workspace policy, Run Profile, Identity Profile, Environment, run, artifact, event, and evidence foundations exist. Profile inventory and full UI coverage are not complete. |
| 02 Electron App Architecture | SPEC DONE | IMPLEMENTATION DONE for M1 | `electron/main/main.ts`, `electron/preload/preload.ts`, `electron/main/ipc.ts`, `src/lib/workflowApi.ts`, and `src/vite-env.d.ts` implement isolated renderer, typed preload API, IPC routing, run event forwarding, and Tauri fallback. Verified by `src/lib/workflowApi.electron.test.ts`, `npm run build`, and `npm run build:electron`. |
| 03 Data And Storage | SPEC DONE | IMPLEMENTATION DONE for M1 | `electron/main/storage.ts` initializes the new SQLite workspace schema, workflows, active graph versions, workspace allowed-origin policy, run profiles, environments, run records with terminal status, run events, artifacts, evidence records, identity profiles, soft delete, duplicate, and temporary settings snapshots. Verified by `electron/main/storage.test.ts`. Full secret references, import/export, retention cleanup, run history UI queries, and profile inventories remain. |
...
| 06 CloakRunner | SPEC DONE | PARTIAL | `electron/runner/runnerCore.ts` executes runner plans with structured lifecycle/step/issue/artifact events, cancellation between actions, screenshot artifacts, allowlist checks, runner artifact path isolation, workspace `maxConcurrency` enforcement before runner start, fingerprint preflight gating, runtime action retries with `action.retrying` events, and runner-level action timeout failure with `action.timeout`. App API forwards Stop to supervised `cancelRun` and maps supervised runner exceptions to failed terminal runs; `runnerSupervisor` supports JSONL `startRun` event streaming, `cancelRun`, non-cooperative cancel kill/restart, and terminal results through `electron/runner/stdioRunner.ts`. Downloads, traces, deeper crash cleanup, and real CloakBrowser smoke remain. |
| 07 Identity Profile And Fingerprint Preflight | SPEC DONE | PARTIAL | `identity_profiles` now has storage CRUD, app/preload API surface, renderer bridge wrappers, filesystem-safe persistent profile slug validation, mobile viewport/touch coherence checks, raw proxy-secret rejection, workflow default identity selection, active-run locking and availability checks for shared persistent profile paths, runner snapshot resolution from saved profiles without raw proxy passwords, and runner preflight policy execution that requires a non-empty owned allowlist and blocks malformed or failed owned verdicts before workflow actions. Full UI wiring, run-profile selection, and real owned probe smoke remain. |
| 08 Run Evidence And Audit | SPEC DONE | PARTIAL | Run events, compact action trace events, terminal run status, run history records, safe artifact metadata, evidence records, recursive export sanitization, compact run evidence export with summary/snapshots/manifest, app/preload API surfaces, preflight verdict evidence persistence, persistent profile lock lifecycle events, strict screenshot artifact policy behavior, and operator Stop/export audit evidence are implemented. Evidence viewer UI, broader operator audit trail, evidence write failure handling, and full export package flow remain. |
| 09 UI/UX Feature Parity | SPEC DONE | PARTIAL | Existing React UI can call Electron preload APIs for workflow, graph, settings, workflow default profile/environment selection, run foundation commands, run history, run profiles, environments, identity profiles including availability checks, workspace policy with Settings editor, evidence summary/export foundations, and run event subscriptions, with a compact run event monitor in workflow detail. Full Electron-native parity for run-profile editor, environment editor, identity editor, evidence viewer, import/export, and issue panel remains. |
