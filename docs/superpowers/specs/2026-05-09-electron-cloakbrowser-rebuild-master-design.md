...
| M2 Runner Vertical Slice | PARTIAL | App API can create a workflow, compile a simple graph, execute runner core events, enforce navigation allowlist in runner tests, register screenshot artifacts, route `startRun` through the supervised JSONL runner process with streamed events, and forward persisted run events to the renderer over preload subscription. Real CloakBrowser-backed navigate/fill/click/wait smoke and user-facing run monitor UI remain. |
...
| 06 CloakRunner | SPEC DONE | PARTIAL | `electron/runner/runnerCore.ts` executes runner plans with structured lifecycle/step/issue/artifact events, cancellation between actions, screenshot artifacts, allowlist checks, fingerprint preflight gating, runtime action retries with `action.retrying` events, and runner-level action timeout failure with `action.timeout`. `runnerSupervisor` supports JSONL `startRun` event streaming, `cancelRun`, and terminal results through `electron/runner/stdioRunner.ts`. Downloads, traces, forceful cleanup, and real CloakBrowser smoke remain. |
