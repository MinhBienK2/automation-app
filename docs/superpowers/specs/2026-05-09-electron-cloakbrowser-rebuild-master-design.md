...
| 09 UI/UX Feature Parity | SPEC DONE | PARTIAL | Existing React UI can call Electron preload APIs for workflow, graph, settings, run foundation commands, run history, identity profiles, and evidence export foundations. Full Electron-native parity for identity editor, evidence viewer, import/export, issue panel, and streamed events remains. |
