...
| 06 CloakRunner | SPEC DONE | PARTIAL | `electron/runner/runnerCore.ts` executes runner plans with structured lifecycle/step/issue/artifact events, cancellation between actions, screenshot artifacts, allowlist checks, fingerprint preflight gating, runtime action retries with `action.retrying` events, and runner-level action timeout failure with `action.timeout`. App API forwards Stop to supervised `cancelRun`; `runnerSupervisor` supports JSONL `startRun` event streaming, `cancelRun`, and terminal results through `electron/runner/stdioRunner.ts`. Downloads, traces, forceful cleanup, and real CloakBrowser smoke remain. |
