...
| 09 UI/UX Feature Parity | SPEC DONE | PARTIAL | Existing React UI can call Electron preload APIs for workflow, graph, settings, run foundation commands, run history, identity profiles, workspace policy, evidence export foundations, and run event subscriptions. Full Electron-native parity for identity editor, evidence viewer, import/export, issue panel, policy editor, and user-facing streamed run monitor remains. |
