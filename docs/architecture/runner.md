...
- CloakBrowser `humanize` defaults to enabled for both temporary and persistent contexts and follows the selected session Browser Launch toggle. The Browser Launch `human_preset` value maps to CloakBrowser `humanPreset` (`default` or `careful`).
- `BrowserSessionManager` maps selected project saved-session or private workflow-session Browser Launch identity values to CloakBrowser launch options before the first page action. The mapping includes stable fingerprint seed, fingerprint fonts directory, persistent profile directory, proxy server/bypass/credentials, explicit timezone/locale or detected local machine timezone/locale, GeoIP, supported WebRTC fingerprint args, humanization timing profile, headless mode, and the lab-verified CloakBrowser mitigation flags `--fingerprint-noise=false`, `--fingerprint-storage-quota=500`, and `--fingerprint-platform=windows`. New project saved sessions and private workflow sessions enable GeoIP by default and blank legacy location settings normalize back to GeoIP; disabling GeoIP requires explicit timezone and locale values. It does not send empty `userAgent`, explicit Playwright launch `viewport`, `--window-size`, or CloakBrowser screen-size overrides. Persona viewport/window dimensions remain in sanitized evidence. Unsupported WebRTC disable policies are rejected by backend settings validation or normalized to `default` when persisted JSON is loaded. In-run Set Viewport changes only width and height.
...
- Command handlers compile the saved graph and pass persisted settings, selected
  project saved-session/private workflow-session Browser Launch settings, and the compiled graph to the run
...
- Browser launch settings come from the selected project saved session or
  private workflow session. `browser_launch.headless` switches CloakBrowser
  between headed and headless mode.
...
- Browser recorder sessions also launch through `BrowserSessionManager`, but
  ...
  uses the same selected-session browser identity baseline, injects page-side
...
- Generated screenshots, text-file artifacts, and downloads are written under `evidence/runs/<run_id>/...` and mirrored in outputs under both compact output keys and structured `__evidence` metadata.
...
- Pointer: click, hover, double click, and drag/drop dispatch CloakBrowser-patched locator/frame primitives where possible. Drag/drop resolves each endpoint from either its direct locator (`source_target`/`target_target` or legacy XPath) or a runtime Find Element ref (`source_ref`/`target_ref`) before readiness checks. Drag/drop with no `target_position`, or with `target_position.mode: "center"`, keeps the native `dragTo` path; percent and pixel-offset target positions resolve source/target bounding boxes and use driver mouse move/down/up so workflows can drag to a precise point inside a target such as a slider track. Targetable single-target actions can consume a runtime `target_ref` produced by Find Element, resolving the original locator plus selected index before dispatch. Custom Select resolves its trigger from `trigger_ref` or direct trigger locator before clicking the option text. Blur Element resolves the configured target/ref and runs locator-side blur on that element rather than tabbing away from the current focus. Right click uses a custom human move plus right-button down/up because the installed CloakBrowser human patch does not preserve the right-button option on its patched click path.
...
- Data capture: text, attribute, input value, list/table, regex matches from prior outputs, screenshot, text-file artifacts, download, and JavaScript outputs. Extract Table reads table rows and `th`/`td` cells through locator-side DOM evaluation. Extract Regex Matches reads the output store and stores a match list without running browser-side script. Write Text File writes a run-scoped text artifact from output values. Execute JavaScript treats the script text as a browser-side function body, so `return ...` scripts can store values through `output_name`. Screenshot, text-file, and download artifacts are run-scoped.
