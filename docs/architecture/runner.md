...
- Scroll: Page mode uses an isolated custom human wheel implementation with bounded random chunks, pauses, and occasional small reverse corrections because CloakBrowser does not patch `page.mouse.wheel` directly; `window.scrollBy` is only a fallback for driver adapters without wheel input. Scroll To Element and Wait Then Scroll To Element resolve the target and use app-owned wheel timing: bounded random chunks, pauses, and small reverse corrections keep the movement goal-directed while avoiding direct `scrollIntoViewIfNeeded` jumps.
