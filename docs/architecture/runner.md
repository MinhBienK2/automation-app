...
- `BrowserSessionManager` maps Workflow Settings Browser Launch identity values to CloakBrowser launch options before the first page action. The mapping includes stable fingerprint seed, fingerprint fonts directory, persistent profile directory, proxy server/bypass/credentials, explicit timezone/locale or detected local machine timezone/locale, GeoIP, supported WebRTC fingerprint args, humanization timing profile, and headless mode. Blank timezone/locale fields use local machine values when GeoIP is off; blank fields are omitted when GeoIP is on. It does not send empty `userAgent`, explicit launch `viewport`, or persona-derived `--window-size` overrides; CloakBrowser's fingerprint seed owns those launch-level device signals. Persona viewport/window dimensions remain in sanitized evidence and preflight comparison. Unsupported WebRTC disable policies are rejected by backend settings validation or normalized to `default` when persisted JSON is loaded. In-run Set Viewport changes only width and height.
