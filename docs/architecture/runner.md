...
- `electron/backend/browser/fingerprintRegression.ts`
- `electron/backend/browser/fingerprintRegression.test.ts`
...
- `BrowserSessionManager` maps Workflow Settings Browser Launch identity values to CloakBrowser launch options before the first page action. The mapping includes stable fingerprint seed, selected persona viewport/window dimensions, fingerprint fonts directory, persistent profile directory, proxy server/bypass/credentials, timezone, locale, GeoIP, supported WebRTC fingerprint args, humanization timing profile, and headless mode. Headed launches include the selected persona Chromium `--window-size=<width>,<height>` argument. Unsupported WebRTC disable policies are rejected by backend settings validation or normalized to `default` when persisted JSON is loaded. In-run Set Viewport changes only width and height.
...
- Optional owned fingerprint preflight runs after launch and initial environment setup but before graph actions. It opens the allowlisted probe URL, parses a structured JSON verdict, recursively redacts sensitive nested evidence keys such as token, cookie, authorization, password, header, IP, account, email, and secret fields, stores sanitized `fingerprint_preflight` evidence, generates `fingerprint_regression` checks for navigator fields, canvas/WebGL/audio hashes, fonts, timezone/locale, WebRTC, display dimensions, proxy geo, storage continuity, and automation/headless leaks, and fails the run before graph execution on blocked or malformed verdicts.
- `BrowserSessionManager` provides the sanitized `browser_identity` record with run id, identity id/display name, profile directory or temporary marker, selected persona id/label/rationale and population buckets, fingerprint seed hash, configured fingerprint font hash when a readable font bundle is present, non-secret proxy label/region/provider/test-account binding, timezone/locale and source, GeoIP/supported WebRTC policy, active advanced override names such as `fingerprint_fonts_dir`, configured humanization status and preset, and CloakBrowser wrapper/binary evidence.
...
- Browser context: Set Viewport changes runtime width and height only; Browser Launch viewport/window defaults come from the selected persona catalog instead of freeform launch overrides. Browser Launch does not expose device scale factor, mobile mode, or touch capability controls.
