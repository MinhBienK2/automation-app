...
- Browser recorder sessions also launch through `BrowserSessionManager`, but
  ...
  events when an adapter binding cannot call back, stores raw recording events
  in memory, and closes the recorder context on stop or discard. Recorder
  normalization maps only to existing runner actions; upload replay requires
  reviewer-entered local file paths, and native file chooser captures remain
  warnings until reviewed.
...
- Scroll: Page mode uses an isolated custom human wheel implementation because CloakBrowser does not patch `page.mouse.wheel` directly; `window.scrollBy` is only a fallback for driver adapters without wheel input. Page mode derives a small number of decisive chunks from the requested pixel distance while preserving the exact total, then emits each chunk as smaller eased wheel pulses with short random pauses inside the gesture and longer random pauses between gestures. Scroll To Element and Wait Then Scroll To Element resolve the target and use app-owned wheel timing: chunk size and pause duration are recalculated from the remaining distance, and the wheel delta points down or up based on the target's position relative to the viewport. Movement stays monotonic and goal-directed while avoiding direct `scrollIntoViewIfNeeded` jumps.
