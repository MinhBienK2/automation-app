...
- Electron runner core consumes compiled `RunPlan` payloads, enforces origin allowlists for navigation, emits deterministic lifecycle/step/issue/artifact events, supports cooperative cancellation between actions, resolves artifact paths under main-allocated directories before capture/write, and rejects path traversal before browser capture.
- Electron app API resolves workspace allowed-origin policy from storage into the runner operator policy snapshot before each run and enforces workspace `maxConcurrency` before creating a run.
- Electron app API locks each active persistent identity profile path before creating a run, records profile lock acquired/released lifecycle events, and releases the lock in run cleanup, so concurrent runs cannot share the same browser profile directory.
- Electron runner core can execute an identity preflight policy before workflow actions by requiring a non-empty owned probe allowlist, navigating to the allowlisted probe URL, reading a JSON verdict from the page body or configured locator, emitting preflight events, and failing the run before graph steps when the verdict is malformed or blocking.
...
- If a supervised runner `startRun` call throws, Electron app API records a system issue plus `run.failed`, finishes the run as failed, and returns a failed run state instead of leaving the run active.
- If `cancelRun` does not respond before the supervisor request timeout, `RunnerSupervisor` kills the non-cooperative runner and the next request starts a fresh process.
...
- Electron app Stop also writes sanitized `operator_action` evidence before forwarding cancellation.
- Remaining Electron runner gaps are real CloakBrowser smoke coverage for navigate/fill/click/wait and preflight, downloads/traces, and deeper browser cleanup after runner crash.
