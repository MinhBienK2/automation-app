...
- Navigation-only headed workflows that use a persistent profile, no proxy, no preflight, default WebRTC, no explicit timezone/locale/GeoIP, and no Custom fingerprint overrides use a passive launch path. The runner starts the browser directly with the target URL and does not attach Playwright/CDP, preserving the saved profile while reducing automation-only signals for owned fingerprint probes. If `CLOAKBROWSER_BINARY_PATH` is set, this path honors that binary and omits the CloakBrowser `--no-sandbox` default; otherwise it launches the CloakBrowser-managed Chromium binary.
...
- Browser sessions are retained in the Electron runner after terminal outcomes unless Workflow Settings Run Policy browser retention is `close` or a compiled terminal Stop Workflow config requests browser closure. Runner-owned outputs are copied into run state before retention or closure; legacy page-side `window.__wamOutputs` is read only when an executed script explicitly referenced that store.
- Passive navigation-only runs retain a lightweight process-backed browser session. It can be closed by the runner, but it does not expose DOM evaluation or run-from-selected continuation because no CDP connection exists.
