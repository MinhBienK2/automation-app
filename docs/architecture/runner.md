...
- Pointer: click, hover, double click, and drag/drop dispatch CloakBrowser-patched locator/frame primitives where possible. Targetable single-target actions can consume a runtime `target_ref` produced by Find Element, resolving the original locator plus selected index before dispatch. Right click uses a custom human move plus right-button down/up because the installed CloakBrowser human patch does not preserve the right-button option on its patched click path.
