...
- `BrowserWorkflowRunner` maps Workflow Settings Browser Launch identity values to CloakBrowser launch options before the first page action. The mapping includes stable fingerprint seed, persistent profile directory, proxy server/bypass/credentials, timezone, locale, GeoIP, viewport dimensions, supported WebRTC fingerprint args, and headless mode. CloakBrowser seed-derived platform, hardware, memory, storage, screen, GPU, device, and font defaults are not exposed as manual Settings overrides and are not re-sent as Playwright context emulation options. Unsupported WebRTC disable policies are rejected by backend settings validation or normalized to `default` when persisted JSON is loaded. In-run Set Viewport changes only width and height.
...
- Browser context: Set Viewport changes runtime width and height only. Low-level launch device and fingerprint values are generated by CloakBrowser from the stable identity seed.
...
- Data capture: text, attribute, input value, list/table, screenshot, download, and JavaScript outputs. Extract Table reads table rows and `th`/`td` cells through locator-side DOM evaluation. Execute JavaScript treats the script text as a browser-side function body, so `return ...` scripts can store values through `output_name`. Screenshot and download artifacts are run-scoped. Download acceptance relies on Playwright's default accepted-download behavior, and the runner saves each download into the run evidence directory through `download.saveAs`.
