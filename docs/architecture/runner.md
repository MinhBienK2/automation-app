...
- `set_variable` accepts typed rows, renders templates, and stores objects natively. Variables are resolved dynamically at runtime using deep path lookups (`getDeepValue`) without flattening objects into duplicate keys. `set_json_variables` parses JSON and writes each top-level key into the output store.
