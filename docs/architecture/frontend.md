...
- `src/features/workflows/components/WorkflowSettingsDialog.tsx`: per-workflow settings dialog with General, Run Policy, Browser Launch, Graph, Environment, and section help. Run Policy shows batch defaults as paused, disabled controls until Batch Run UI is ready.
...
- Workflow Settings editing through list Edit and detail Settings, simplified Run Policy with paused read-only batch defaults, Browser Launch, Graph link-wait authoring defaults, Environment initial variables, dialog-level saving for all dirty sections, unsaved-close confirmation, bilingual section help with field-level guidance, and run-before-save orchestration.
