...
- `src/features/workflows/components/RunProgressNavigator.tsx`: workflow-detail live run navigator derived from the shared run-state fields. It shows the current/failed node, recent completed/current trail, Follow current, and Focus current/failed node controls.
...
- Workflow detail run monitoring includes a live run navigator above the graph canvas when the current run has active or recent progress. It derives labels from the loaded graph plus `current_step_id`, `current_step_number`, `completed_step_ids`, and `error.step_id`, exposes a bounded recent execution trail, can select/focus the current or failed node, and auto-follows current-node changes while Follow current is enabled. It does not invent branch, loop, retry, or skipped-node events beyond the current run-state contract.
