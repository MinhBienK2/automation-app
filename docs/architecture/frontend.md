...
- `src/features/workflows/components/WorkflowSettingsDialog.tsx`: per-workflow settings dialog with General, Graph, Run Policy, Browser Launch, Environment, grouped fieldsets for related controls, and section help. Browser Launch keeps seed-derived fingerprint values in an optional Custom fingerprint overrides group. Run Policy shows batch defaults as paused, disabled controls until Batch Run UI is ready.
...
- Workflow Settings editing through list Edit and detail Settings, grouped related controls within each section, simplified Run Policy with paused read-only batch defaults, Browser Launch with optional Custom fingerprint overrides, Graph link-wait authoring defaults, Environment initial variables, dialog-level saving for all dirty sections, unsaved-close confirmation, bilingual section help with field-level guidance, and run-before-save orchestration.
