...
- `src/features/workflows/components/WorkflowBrowserConfigPanel.tsx`: workflow-level browser runtime config form rendered from the workflow detail Runtime dialog.
...
- Workflow browser runtime config editing through the workflow detail header Runtime dialog, save state, and run-before-save orchestration.
