...
- Command invocation through `workflowApi.ts`. New Electron-compatible calls should be added to the preload surface and wrapper fallback together. Identity Profile wrappers currently expose list/get/create/update/delete/validate for the Electron rebuild contract. Evidence wrappers expose event/artifact listing, sanitizer access, and compact sanitized run export.
