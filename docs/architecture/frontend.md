...
- `src/features/workflows/components/RunProgressNavigator.tsx`: workflow-detail live run navigator derived from the shared run-state fields. It is rendered only when saved Graph settings enable Live Run, initializes Follow current from the saved Graph setting, and shows the current/failed node, recent completed/current trail, Follow current, and Focus current/failed node controls.
...
- Workflow detail run monitoring includes a live run navigator above the graph canvas when saved Graph settings enable Live Run and the current run has active or recent progress. It derives labels from the loaded graph plus `current_step_id`, `current_step_number`, `completed_step_ids`, and `error.step_id`, exposes a bounded recent execution trail, can select/focus the current or failed node, and initializes auto-follow from the saved Follow current setting while still allowing the navigator toggle to override the current page session. It does not invent branch, loop, retry, or skipped-node events beyond the current run-state contract.
