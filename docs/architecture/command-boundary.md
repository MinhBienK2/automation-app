...
- Workflow Settings validation is command-owned and emits fingerprint-coherence warnings for proxy identities without timezone/locale or GeoIP, custom user agents that can diverge from UA Client Hints, shared fingerprint fonts directories that can create stable font hashes, and macOS fingerprint profiles that need owned preflight due to upstream CloakBrowser caveats.
