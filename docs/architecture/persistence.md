...
- Workflow Settings are stored in `workflow_settings` keyed by `workflow_id`, with version plus JSON columns for General, Execution, Browser, Behavior, Environment, Inputs, Triggers, and Advanced. The persisted Inputs column remains the compatibility storage for the UI's Variables section.
