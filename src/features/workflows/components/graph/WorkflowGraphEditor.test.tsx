...
const workflowGraphInspectorSource = readFileSync(
  join(process.cwd(), "src/features/workflows/components/inspector/WorkflowGraphInspector.tsx"),
  "utf8",
);
const workflowGraphShortcutsSource = readFileSync(
  join(process.cwd(), "src/features/workflows/components/hooks/useWorkflowGraphShortcuts.ts"),
  "utf8",
);
const workflowGraphToolbarSource = readFileSync(
  join(process.cwd(), "src/features/workflows/components/graph/WorkflowGraphToolbar.tsx"),
  "utf8",
);
...
const graphNodeTypeCoverage: Record<GraphNodeType, true> = {
  ...
  set_number_variable: true,
  generate_random_number: true,
  parse_text_to_number: true,
  math_operation: true,
  round_number: true,
  format_number: true,
  compare_numbers: true,
  check_number_range: true,
  check_number_property: true,
  set_text_variable: true,
  append_text: true,
  prepend_text: true,
  replace_text: true,
  trim_text: true,
  change_text_case: true,
  slice_text: true,
  regex_extract: true,
  get_text_length: true,
  check_text_empty: true,
  check_text_contains: true,
  check_text_regex_matches: true,
  update_text_variable: true,
  update_flag_variable: true,
  set_boolean_variable: true,
  generate_random_boolean: true,
  parse_to_boolean: true,
  boolean_logical_op: true,
  compare_booleans: true,
  check_boolean_property: true,
  ...
  create_object_manual: true,
  parse_json_to_object: true,
  set_object_property: true,
  remove_object_property: true,
  merge_objects: true,
  rename_object_property: true,
  get_object_property: true,
  get_object_keys: true,
  get_object_values: true,
  stringify_object: true,
  execute_object_script: true,
  check_object_key_exists: true,
  check_object_empty: true,
  assert_output: true,
  domain_allowlist: true,
  get_current_url: true,
  extract_text: true,
  extract_attribute: true,
  extract_input_value: true,
  extract_table: true,
  extract_list: true,
  count_elements: true,
  extract_regex_matches: true,
  extract_text_content: true,
  extract_inner_html: true,
  extract_outer_html: true,
  extract_computed_style: true,
  extract_all_attributes: true,
  extract_data_attributes: true,
  extract_class_list: true,
  extract_descendant_attributes: true,
  extract_select_value: true,
  extract_select_options: true,
  extract_checkbox_state: true,
  extract_form_data: true,
  extract_table_headers: true,
  extract_table_row: true,
  extract_table_column: true,
  extract_table_cell: true,
  extract_list_attributes: true,
  extract_structured_list: true,
  extract_dimensions: true,
  extract_visibility: true,
  extract_element_state: true,
  check_element_exists: true,
  get_page_title: true,
  get_meta_content: true,
  extract_page_links: true,
...
describe("Workflow graph editor integration", () => {
  ...
  // Inspector-heavy flows can exceed the default 20s on loaded dev machines.
  ...
  test("inserts variables discovered from graph variable nodes into template fields", async () => {
    ...
    await userEvent.click(
      within(screen.getByRole("listbox", { name: "Text variable options" })).getByText("token"),
    );
