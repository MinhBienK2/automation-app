...
describe("ActionConfigEditor", () => {
  ...
  test("inserts variable tokens into template fields at the cursor", async () => {
    ...
    const listbox = screen.getByRole("listbox", { name: "Text variable options" });
    expect(listbox).toBeInTheDocument();
    expect(within(listbox).queryByText("user.name")).not.toBeInTheDocument();

    await userEvent.click(within(listbox).getByText("last_error"));
  ...
  test("shows graph variable options in template fields", async () => {
    ...
    const listbox = screen.getByRole("listbox", { name: "Text variable options" });
    expect(within(listbox).getByText("Set Variables")).toBeInTheDocument();
    expect(within(listbox).getByText("account_id")).toBeInTheDocument();
  });
