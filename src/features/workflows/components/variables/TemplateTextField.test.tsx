...
describe("TemplateTextField", () => {
  ...
  test("renders textarea and handles variable insertion via popover", async () => {
    ...
    const listbox = screen.getByRole("listbox", { name: "My Textarea variable options" });

    // user.name and roles should NOT be present in options anymore
    expect(within(listbox).queryByText("user.name")).not.toBeInTheDocument();
    expect(within(listbox).queryByText("roles")).not.toBeInTheDocument();

    // system.loop.index should be present
    expect(within(listbox).getByText("index")).toBeInTheDocument();

    // Click an option in the portal popover (system.last_error)
    await user.click(within(listbox).getByText("last_error"));
  ...
  test("inserts variable in JavaScript format when isJs is true", async () => {
    ...
    const listbox = screen.getByRole("listbox", { name: "JS Textarea variable options" });

    // Tree renders raw variable names; the outputs. prefix applies on insert
    expect(within(listbox).getByText("my_variable")).toBeInTheDocument();
    expect(within(listbox).getByText("index")).toBeInTheDocument();

    // Click the standard variable option
    await user.click(within(listbox).getByText("my_variable"));
