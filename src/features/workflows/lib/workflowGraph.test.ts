...
describe("workflow graph helpers", () => {
  ...
  test("creates default nodes with stable ports", () => {
    ...
    const updateVariableNode = createDefaultGraphNode("update_number_variable", { x: 35, y: 45 });
    ...
    expect(updateVariableNode.label).toBe("Update Number Variable");
  ...
  test("returns stable port definitions for graph node types", () => {
    ...
    expect(nodePorts("update_number_variable").map((port) => port.id)).toEqual([
      "in",
      "out",
    ]);
