...
describe("Workflow detail integration", () => {
  ...
  test("does not expose workflow session selection or fork controls", async () => {
    ...
    await openWorkflows();
    const workflowCard = (await screen.findByRole("heading", { name: "Login flow" }))
  ...
  test("retains finished run state and variables when exiting and reopening if the browser session is still open", async () => {
    const selectedWorkflow = {
      ...workflow,
      browser_profile_id: "environment-qa",
    };
    ...
    // Check that completed run variables ARE still present because retained_session.available was true!
    expect(within(newVariablesDrawer).getByText("new_var")).toBeInTheDocument();
    expect(within(newVariablesDrawer).getByText("run_val")).toBeInTheDocument();
    expect(within(newVariablesDrawer).getByText("settings_var")).toBeInTheDocument();
    expect(within(newVariablesDrawer).getByText("profile_var")).toBeInTheDocument();
  });
});
