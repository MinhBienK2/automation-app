...
export class IdentityRepository {
  ...
  async getDetail(
    ...
    diagnostics = await this.options.diagnostics(),
    ...
    const latestObservedOutput = latestObservedRun
      ...
      : null;
    const profileDiagnostics = diagnostics.profiles.find(
    ...
    return {
      ...
      diagnostics: {
        binary_installed: diagnostics.binary.installed,
        wrapper_version: diagnostics.wrapper_version,
        geoip_available: diagnostics.geoip_available,
        headed_display_available: diagnostics.headed_display.available,
        profile: profileDiagnostics
          ...
          : null,
        font_status: diagnostics.font_checklist.status,
      },
      actions: {
  ...
  private managedSummary(workflow: WorkflowSummary): ManagedIdentitySummary | null {
    ...
    return {
      ...
      retained_session: { active: retained?.available === true, reason: retained?.reason ?? null },
