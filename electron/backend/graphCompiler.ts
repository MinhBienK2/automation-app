...
export function validateActionConfig(config: ActionConfig): ValidationError | null {
  switch (config.type) {
    ...
    case "input_text":
    case "clear_input":
      return hasElementTarget(config.config) ? null : validationError("xpath", "Element target is required");
    case "set_variable": {
...
function settingsPreludeSteps(settings: WorkflowSettings): CompiledGraphStep[] {
  ...
  if (settings.environment.download_directory) {
    steps.push(settingsStep("environment:downloads", "Apply settings download directory", {
      type: "set_download_directory",
      config: { path: settings.environment.download_directory },
    }));
  }
...
function hasElementTarget(config: unknown): boolean {
  const record = asRecord(config);
  return Boolean(
    (typeof record.xpath === "string" && record.xpath.trim()) ||
      record.target,
  );
}
