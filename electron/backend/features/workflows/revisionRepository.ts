...
async function getSubflowSnapshotsForWorkflow(
  ...
  const subflowIds = [
    ...new Set(
      graph.nodes
        ...
        .map((node: any) => (node.config as { subflow_id?: unknown })?.subflow_id)
        .filter((subflowId: unknown): subflowId is string => typeof subflowId === "string" && subflowId.trim().length > 0)
    )
  ];
  ...
  for (const subflowId of subflowIds as string[]) {
