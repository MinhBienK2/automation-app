/**
 * Control flow: loops, branches, routers, retries, try/catch and stop.
 *
 * These execute *above* any execution surface. They read and write run
 * outputs, ask the caller whether a condition holds, and hand nested steps
 * back to it — and that is the whole reason a second surface is affordable:
 * the most correctness-critical machinery in the runner is surface-independent
 * by construction, and the Desktop Surface inherits it unchanged.
 *
 * The `VariableScope` parameter is what proves it. There is no page here to
 * reach for. See ADR-0001 and `docs/architecture/desktop-runner.md`.
 */
...
export function createControlFlowExecutors<Runtime extends VariableScope>(
  runtime: Runtime,
