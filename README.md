...
1. Use the shell command bar to search a workflow, run, evidence item, schedule, and identity. Confirm results open the correct workspace through typed targets without showing raw outputs, cookies, tokens, proxy credentials, absolute local paths, or browser storage. Click Alerts and confirm Overview opens with Attention Queue focused.
