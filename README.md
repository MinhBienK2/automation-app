...
24. Confirm Workflow Settings Browser Launch identity profile, proxy, timezone/locale/GeoIP, WebRTC, humanize toggle/preset, preflight, and headless defaults apply before browser launch; CloakBrowser seed-derived fingerprint defaults are not manually editable in Settings; Environment initial variables apply before graph actions run; saved graph edge waits run before their target nodes; graph nodes for geolocation, permissions, headers, cookies, and storage apply from the graph when used; and outputs include sanitized `browser_identity` evidence.
