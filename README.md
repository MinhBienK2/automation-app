...
32. Open the workflow graph and confirm a new workflow starts as `Start -> New node`. Confirm the graph toolbar shows New node, Add Action, Add Logic, Add Variable, and Add End without Add Output or toolbar Fit. Add visible graph nodes, choose an action type from the searchable inspector dropdown, edit inspector fields, connect nodes through explicit ports, reconnect a used port and confirm the old link is replaced, multi-select nodes, bulk duplicate/copy/paste/delete, undo/redo graph edits, select/delete a link, validate the graph, save it, reopen the workflow, and confirm the graph persists.
