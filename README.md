...
lazy-created project saved identities and workflow Browser Launch settings
auto-fill that repo-local path when the directory exists and is readable;
operators can still edit or clear Browser Launch -> Fingerprint fonts
directory for the workflow identity. The generated font files and downloaded `.deb` packages are local
