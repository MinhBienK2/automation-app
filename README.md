...
9. Open Settings from the workflow detail header, confirm it opens to Browser Launch, and configure Reuse login session, the workflow browser profile, proxy, and headed/headless default. Confirm Workflow Settings has General, Run Policy, Browser Launch, Environment, and Owned Test Gates sections; Run Policy shows disabled batch controls with the pause note; Environment edits initial variable rows; Owned Test Gates edits fingerprint preflight fields; section help is available; the dialog has one Save Settings button in the header; and closing with edits shows the unsaved-changes prompt.
