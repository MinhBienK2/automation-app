...
1. Confirm the app opens on Overview, shows Active Runs, Succeeded Today, Attention Needed, Upcoming Schedules, Live Operations, Attention Queue, Execution Activity, Recent Evidence, and Upcoming Schedules, and that Open Workflows navigates to the workflow list. Confirm the sidebar order is Overview, Workflows, Runs, Evidence, Schedules, Identities, Settings.
1. Use the shell command bar to search a workflow, run, evidence item, schedule, and identity. Confirm grouped command palette results open the correct workspace through typed targets without showing raw outputs, cookies, tokens, proxy credentials, absolute local paths, or browser storage. Confirm `/` and Ctrl/Meta+K do not steal focus while an overlay is open. Click Alerts, confirm the preview opens, press `Esc` to close it, then reopen Alerts and choose Open Attention Queue to confirm Overview opens with Attention Queue focused.
1. Open Evidence from the sidebar, confirm screenshot/download/browser identity/action trace/evidence manifest items from completed runs are searchable and filterable, active filters and list/grid modes are visible, typed empty states appear for no evidence/no matches/unavailable focused evidence, screenshot preview and Reveal in Folder work only through validated evidence actions, downloads show metadata without in-app preview, browser identity/action trace details omit sensitive fields, Export Selection shows selected/exported/omitted counts without absolute original paths, and Overview Recent Evidence plus Runs selected details navigate into Evidence.
...
9. Open Settings from the workflow detail header, confirm it opens to Browser Launch, and configure Reuse login session, identity display name, stable identity id, read-only fingerprint seed, fingerprint fonts directory, proxy URL/credentials/bypass/metadata, timezone/locale with GeoIP location enabled by default and the detected local machine fallback visible, Humanize browser input, Humanize preset (`default` or `careful`), and headed/headless default. Confirm Browser Launch shows bounded persona/OS/browser/font identity posture and warnings for GeoIP/proxy/font-directory risk states. Confirm Reset identity opens an in-app confirmation that names the workflow/current identity, explains pending settings are saved first, returns a backend-generated identity/profile/seed, preserves historical runs/evidence, and disables Run from selected. Confirm Workflow Settings has General, Graph, Run Policy, Browser Launch, and Environment sections with related fields grouped inside each section; the header shows workflow context, dirty state, errors, and one Save Settings button; Run Policy edits max duration, browser retention, Allow Run JavaScript, and a grouped Run from selected control with the Run from selected scope select, and shows disabled batch controls with the pause note; Graph edits the new link wait default in one grouped control; Environment edits initial variable rows; section help is available with nested collapsible guidance groups and item-level field/example/mistake disclosures; and closing with edits shows the unsaved-changes prompt.
12. From the Workflow Library, confirm the table/detail workspace selects a workflow by default, search/filter/sort stays local, row `Open Graph` opens the graph workspace, direct Run keeps the app on the list, active rows switch to Stop, and Duplicate, Export, and Delete are disabled only for rows whose workflow is already running.
13. From Graph Builder, click `Launch Run`, confirm the dialog names the workflow, graph save state, current browser identity/session context, visible-graph save, dirty Workflow Settings save, validation, and browser-launch checks; confirm known blocking issues show only a compact warning that points back to Validate/Run issues; cancel once to verify no run starts, then confirm launch to verify the existing save/settings/validation/run pipeline starts. Open Runs from the sidebar, confirm newest session runs list workflow/source/status/step/started/issue/action columns, row Stop targets the selected run id, selected persisted run details stay bounded to metadata/actions, stale run targets keep the table visible, and no raw outputs or artifacts are shown.
12. Open Schedules from the sidebar, confirm summary counts/readable last
   decisions render, create disabled one-time, interval, daily, weekly, and
   monthly schedules for saved workflows, enable valid schedules, confirm
   invalid workflow drafts cannot be enabled and errors stay in the schedule
   row or dialog, confirm delete requires a named confirmation, confirm
   isolated due schedules can start concurrently, and confirm schedule history
   records started/skipped/missed/failed-to-start/disabled decisions with
   workflow/profile/batch conflict reasons when skipped. Confirm schedule
   history safely summarizes validation details, can open related Runs and
   Workflows, and stale schedule/run targets show unavailable-target states.
13. From the Workflow Library More menu/detail panel, duplicate a workflow and confirm the dialog explains the `Copy of <name>` result, copied graph/settings, fresh browser identity/profile/fingerprint, and disabled Run from selected. Export a workflow package with Flow and selected Settings, confirm the sanitization note and native Save dialog, import it back as a new workflow without overwriting or leaving a partial workflow, delete with profile data kept by default, and run a graph-backed batch with at least two input rows.
...
14. From the workflow list, click Record Workflow, record common form actions including click, text entry, select, checkbox/radio, clipboard paste, upload, and submit, stop the backend-owned recorder session, review the generated draft summary and needs-attention filters, edit a step label and clipboard text, enter required upload file paths, exclude one generated step, confirm discard/close is guarded, save it as a new workflow, reopen it, and confirm the saved graph runs through normal workflow validation/run controls with recorded inter-step pacing represented as edge waits. From a workflow detail page, click Record Replacement, stop recording, confirm the review action says Replace Graph and explains that only the graph is replaced, and save to replace only that workflow graph. Dry-run validate a config, and in a debug build generate a local fixture HTML file from a single `.html` filename.
15. Move graph nodes by dragging the node body, clear selection to see graph health, select Start to confirm protected delete/copy messaging, reopen the workflow, and confirm node positions persist.
