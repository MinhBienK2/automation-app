...
9. Open Settings from the workflow detail header, confirm it opens to Browser Launch, and configure Reuse login session, identity display name, stable identity id, fingerprint seed, proxy URL/credentials/bypass/metadata, timezone/locale, viewport/device flags, allowlisted advanced fingerprint overrides, Humanize browser input, Humanize preset (`default` or `careful`), optional owned fingerprint preflight, and headed/headless default. Confirm Workflow Settings has General, Run Policy, Browser Launch, Graph Defaults, and Environment sections; Run Policy shows disabled batch controls with the pause note; Graph Defaults edits the default wait for newly created links; Environment edits initial variable rows; section help is available; the dialog has one Save Settings button in the header; and closing with edits shows the unsaved-changes prompt.
...
12. Open Schedules from the sidebar, create disabled one-time, interval, daily, weekly, and monthly schedules for a saved workflow, enable a valid schedule, confirm invalid workflow drafts cannot be enabled, and confirm schedule history records started/skipped/missed/failed-to-start/disabled decisions.
